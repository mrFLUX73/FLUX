...
3. Скопируйте `.env.example` в `.env.local`, заполните URL, publishable key и публичный site key Turnstile, затем установите `VITE_SUPABASE_ANONYMOUS_AUTH_ENABLED=true`.
...
Workflow `.github/workflows/deploy.yml` публикует папку `dist` после каждого push в ветку `main`. В настройках репозитория GitHub выберите **Pages → Source → GitHub Actions**, а в **Settings → Secrets and variables → Actions → Variables** добавьте `VITE_SUPABASE_URL`, `VITE_SUPABASE_PUBLISHABLE_KEY`, публичный `VITE_TURNSTILE_SITE_KEY` и, только после настройки защиты, `VITE_SUPABASE_ANONYMOUS_AUTH_ENABLED=true`.
