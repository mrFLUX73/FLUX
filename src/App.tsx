...
function ClientOverviewDrawer({ open, onOpenChange, userId, link, onOpenChat, onRevoke }: { open: boolean; onOpenChange: (open: boolean) => void; userId: string; link: TrainerLink | null; onOpenChat: () => void; onRevoke: () => void }) {
  ...
  const refresh = useCallback(async (quiet = false) => { if (!link) return; if (!quiet) setLoading(true); setError(''); try { const [nextOverview, nextNutrition, nextWorkouts, nextNotes] = await Promise.all([loadTrainerClientOverview(userId, link.id), loadTrainerClientNutrition(userId, link.id, day), loadTrainerClientWorkouts(userId, link.id), loadTrainerClientNotes(userId, link.id)]); setOverview(nextOverview); setNutrition(nextNutrition); setWorkouts(nextWorkouts); setNotes(nextNotes); } catch { setError('Данные клиента сейчас недоступны. Проверьте связь и попробуйте ещё раз.'); } finally { if (!quiet) setLoading(false); } }, [day, link, userId]);
