...
import { PersonalPlanEditor } from './features/workouts/PersonalPlanEditor';
import { archiveTrainerAssignedWorkoutPlan, deleteTrainerAssignedWorkoutPlan, loadTrainerAssignedWorkoutPlan, loadTrainerAssignedWorkoutPlans, loadWorkoutHistory, type TrainerAssignedPlanSummary, type WorkoutHistoryItem, type WorkoutPlan } from './features/workouts/repository';
...
import { countUnreadTrainerMessages, deleteTrainerClientNote, loadMyTrainerCode, loadTrainerClientNutrition, loadTrainerClientNotes, loadTrainerClientOverview, loadTrainerClientWorkouts, loadTrainerHub, loadTrainerInbox, loadTrainerInvitePreview, loadTrainerMessages, markTrainerMessagesSeen, requestTrainerConnection, respondToTrainerConnection, revokeTrainerConnection, saveTrainerClientNote, sendTrainerMessage, setAccountRole, TrainerConnectionError, type TrainerClientNutrition, type TrainerClientNote, type TrainerClientOverview, type TrainerClientWorkouts, type TrainerInboxLink, type TrainerInvitePreview, type TrainerLink, type TrainerMessage } from './features/trainer/repository';
...
function TrainerAssignedPlans({ userId, link }: { userId: string; link: TrainerLink }) {
  const [plans, setPlans] = useState<TrainerAssignedPlanSummary[]>([]); const [loading, setLoading] = useState(true); const [error, setError] = useState('');
  const [editorOpen, setEditorOpen] = useState(false); const [editorPlan, setEditorPlan] = useState<WorkoutPlan | null>(null);
  const refresh = useCallback(async () => { setLoading(true); setError(''); try { setPlans(await loadTrainerAssignedWorkoutPlans(userId, link.id)); } catch { setError('Не удалось загрузить планы. Возможно, связь с клиентом больше не активна.'); } finally { setLoading(false); } }, [link.id, userId]);
  useEffect(() => { void refresh(); }, [refresh]);
  const edit = async (id: string) => { try { setEditorPlan(await loadTrainerAssignedWorkoutPlan(userId, link.id, id)); setEditorOpen(true); } catch { setError('Не удалось открыть план. Проверьте активность связи с клиентом.'); } };
  const archive = async (plan: TrainerAssignedPlanSummary) => { if (!window.confirm(`Архивировать «${plan.name}»?`)) return; try { await archiveTrainerAssignedWorkoutPlan(userId, link.id, plan.id); await refresh(); } catch { setError('Не удалось архивировать план.'); } };
  const remove = async (plan: TrainerAssignedPlanSummary) => { if (!window.confirm(`Удалить «${plan.name}»? Активная тренировка клиента продолжится по сохранённому snapshot.`)) return; try { await deleteTrainerAssignedWorkoutPlan(userId, link.id, plan.id); await refresh(); } catch { setError('Не удалось удалить план.'); } };
  return <section className="flux-client-sessions"><div className="flux-section-heading"><h2>Планы тренировок</h2><Button size="sm" onClick={() => { setEditorPlan(null); setEditorOpen(true); }}><Plus /> Создать план</Button></div>{loading ? <p className="flux-trainer-empty">Загружаем планы…</p> : error ? <div className="flux-trainer-workspace-state"><p>{error}</p><Button size="sm" onClick={() => { void refresh(); }}>Повторить</Button></div> : plans.length ? <div className="flux-trainer-notes-list">{plans.map((plan) => <article key={plan.id}><p><strong>{plan.name}</strong><small>{plan.exerciseCount} упражнений · {plan.estimatedDurationMinutes ? `${plan.estimatedDurationMinutes} мин` : 'длительность не указана'}{plan.isActive ? '' : ' · архив'}</small></p><span><button type="button" onClick={() => { void edit(plan.id); }} aria-label={`Редактировать ${plan.name}`}><Pencil /></button>{plan.isActive && <button type="button" onClick={() => { void archive(plan); }} aria-label={`Архивировать ${plan.name}`}><X /></button>}<button type="button" onClick={() => { void remove(plan); }} aria-label={`Удалить ${plan.name}`}><Trash2 /></button></span></article>)}</div> : <div className="flux-trainer-empty"><p>У клиента пока нет планов тренировок</p><Button size="sm" onClick={() => { setEditorPlan(null); setEditorOpen(true); }}><Plus /> Создать план</Button></div>}<PersonalPlanEditor userId={userId} plan={editorPlan} open={editorOpen} onOpenChange={setEditorOpen} onSaved={() => { void refresh(); }} context={{ kind: 'trainer_for_client', linkId: link.id, clientName: link.clientName }} /></section>;
}
...
function ClientOverviewDrawer({ open, onOpenChange, userId, link, onOpenChat, onRevoke }: { open: boolean; onOpenChange: (open: boolean) => void; userId: string; link: TrainerLink | null; onOpenChat: () => void; onRevoke: () => void }) {
  ...
  const mealName: Record<string, string> = { breakfast: 'Завтрак', lunch: 'Обед', dinner: 'Ужин', snack: 'Перекус', other: 'Приём пищи' };
  const goal = overview?.calorieGoal ?? null;
  const activeTab = (id: typeof tab, title: string) => <button key={id} type="button" className={tab === id ? 'is-active' : ''} onClick={() => setTab(id)}>{title}</button>;
  return <Drawer open={open} onOpenChange={onOpenChange}><DrawerContent className="flux-drawer flux-client-workspace-drawer"><DrawerHeader className="flux-drawer-header flux-client-workspace-header"><button type="button" className="flux-drawer-back" onClick={() => onOpenChange(false)} aria-label="Назад к клиентам"><ArrowLeft /></button><div><DrawerTitle>{link?.clientName ?? 'Клиент'}</DrawerTitle><DrawerDescription>Активная связь с тренером</DrawerDescription></div><span className="flux-avatar-placeholder">{link?.clientName.slice(0, 1).toUpperCase()}</span></DrawerHeader><div className="flux-client-workspace-body">{loading ? <p className="flux-trainer-empty">Загружаем карточку клиента…</p> : error && !overview ? <div className="flux-trainer-workspace-state"><p>{error}</p><Button size="sm" onClick={() => { void refresh(); }}>Повторить</Button></div> : overview ? <><div className="flux-client-actions"><Button size="sm" onClick={onOpenChat}><MessageCircle /> Написать</Button><button type="button" onClick={() => { setTab('overview'); document.querySelector<HTMLTextAreaElement>('.flux-trainer-notes-compose textarea')?.focus(); }}><Pencil /> Заметка</button><button type="button" onClick={() => setTab('workouts')}><Sprout /> План</button><button type="button" className="is-more" onClick={onRevoke}><X /> Ещё</button></div><div className="flux-client-tabs">{activeTab('overview', 'Обзор')}{activeTab('nutrition', 'Питание')}{activeTab('workouts', 'Тренировки')}{activeTab('progress', 'Прогресс')}</div>{error && <p className="flux-client-inline-error">{error}</p>}{tab === 'overview' && <div className="flux-client-workspace-content"><section className="flux-client-goal-card"><small>Цель клиента</small><strong>{overview.currentWeightKg != null ? `${overview.currentWeightKg} кг сейчас` : 'Вес пока не указан'}</strong><p>{goal ? `${goal} ккал в день · Б ${Math.round(overview.proteinGoal ?? 0)} · Ж ${Math.round(overview.fatGoal ?? 0)} · У ${Math.round(overview.carbsGoal ?? 0)}` : 'Цели питания клиент пока не заполнил.'}</p></section><section className="flux-client-facts"><div><small>Сегодня</small><strong>{Math.round(overview.todayKcal)} <em>/ {goal ?? '—'} ккал</em></strong><p>{overview.mealsToday ? `${overview.mealsToday} приёмов · Б ${Math.round(overview.todayProtein)} · Ж ${Math.round(overview.todayFat)} · У ${Math.round(overview.todayCarbs)}` : 'Сегодня нет записей питания'}</p></div><div><small>Тренировка</small><strong>{overview.workouts7 ? `${overview.workouts7} за 7 дней` : 'Пока нет'}</strong><p>{overview.lastWorkoutAt ? `Последняя: ${new Intl.DateTimeFormat('ru-RU', { day: 'numeric', month: 'short' }).format(new Date(overview.lastWorkoutAt))}` : 'Нет завершённых тренировок'}</p></div></section><section className="flux-trainer-notes"><div className="flux-section-heading"><h2>Приватные заметки</h2><span>{notes.length}</span></div><div className="flux-trainer-notes-compose"><textarea value={noteBody} maxLength={2000} placeholder="Например: не любит рыбу…" onChange={(event) => setNoteBody(event.target.value)} /><Button size="sm" disabled={savingNote || !noteBody.trim()} onClick={() => { void saveNote(); }}>{editingNoteId ? 'Сохранить' : 'Добавить'}</Button></div>{editingNoteId && <button type="button" className="flux-note-cancel" onClick={() => { setEditingNoteId(null); setNoteBody(''); }}>Отменить редактирование</button>}{notes.length ? <div className="flux-trainer-notes-list">{notes.map((note) => <article key={note.id}><p>{note.body}</p><small>{new Intl.DateTimeFormat('ru-RU', { day: 'numeric', month: 'short' }).format(new Date(note.updatedAt))}</small><span><button type="button" onClick={() => editNote(note)} aria-label="Редактировать заметку"><Pencil /></button><button type="button" onClick={() => { void removeNote(note); }} aria-label="Удалить заметку"><Trash2 /></button></span></article>)}</div> : <p className="flux-trainer-empty">Заметки видны только вам.</p>}</section></div>}{tab === 'nutrition' && <div className="flux-client-workspace-content"><label className="flux-client-day-picker">День<input type="date" value={day} onChange={(event) => setDay(event.target.value)} /></label><section className="flux-client-goal-card"><small>Рацион за день</small><strong>{Math.round(nutrition?.kcal ?? 0)} <em>/ {nutrition?.goalKcal ?? '—'} ккал</em></strong><p>Б {Math.round(nutrition?.protein ?? 0)} · Ж {Math.round(nutrition?.fat ?? 0)} · У {Math.round(nutrition?.carbs ?? 0)}</p></section>{nutrition?.meals.length ? nutrition.meals.map((meal) => <section className="flux-client-meal" key={meal.id}><header><strong>{mealName[meal.type] ?? 'Приём пищи'}</strong><time>{new Intl.DateTimeFormat('ru-RU', { hour: '2-digit', minute: '2-digit' }).format(new Date(meal.eatenAt))}</time></header>{meal.items.map((item) => <div key={item.id}><span><b>{item.name}</b><small>{Math.round(item.amountG)} г</small></span><strong>{Math.round(item.kcal)} ккал</strong></div>)}</section>) : <p className="flux-trainer-empty">В этот день записей питания нет.</p>}</div>}{tab === 'workouts' && <div className="flux-client-workspace-content"><TrainerAssignedPlans userId={userId} link={link!} /><section className="flux-client-goal-card"><small>Текущий план клиента</small><strong>{workouts?.plan?.name ?? 'Активного плана нет'}</strong><p>{workouts?.plan ? `${workouts.plan.durationMinutes ? `${workouts.plan.durationMinutes} мин` : 'Длительность не указана'}${workouts.plan.description ? ` · ${workouts.plan.description}` : ''}` : 'Назначение плана тренером будет отдельной функцией — фактические тренировки клиента не изменяются.'}</p></section><section className="flux-client-sessions"><div className="flux-section-heading"><h2>История</h2><span>{workouts?.sessions.length ?? 0}</span></div>{workouts?.sessions.length ? workouts.sessions.map((session) => <div key={session.id}><span className="flux-rhythm-icon"><Dumbbell /></span><p><strong>{session.title}</strong><small>{new Intl.DateTimeFormat('ru-RU', { day: 'numeric', month: 'short' }).format(new Date(session.startedAt))} · {session.status === 'completed' ? 'выполнена' : session.status === 'planned' ? 'запланирована' : session.status}</small></p></div>) : <p className="flux-trainer-empty">Тренировок пока нет.</p>}</section></div>}{tab === 'progress' && <div className="flux-client-workspace-content"><section className="flux-client-goal-card"><small>Текущий вес</small><strong>{overview.currentWeightKg != null ? `${overview.currentWeightKg} кг` : 'Пока нет данных'}</strong><p>{goal ? 'Цели питания доступны в обзоре. Историю веса FLUX пока не хранит отдельно, поэтому график здесь не имитируем.' : 'Когда клиент добавит вес, он появится здесь.'}</p></section><div className="flux-client-progress-empty"><ChartNoAxesColumnIncreasing /><strong>История прогресса появится здесь</strong><p>Покажем график, когда в профиле появятся отдельные записи веса.</p></div></div>}</> : null}</div></DrawerContent></Drawer>;
}
...
export default function App() {
  ...
  return (
    <Toaster>
      ...
      {account && <ClientOverviewDrawer open={Boolean(clientOverviewLink)} onOpenChange={(open) => { if (!open) setClientOverviewLink(null); }} userId={account.id} link={clientOverviewLink} onOpenChat={() => { if (clientOverviewLink) openTrainerChat(clientOverviewLink, 'client-overview'); }} onRevoke={() => { if (clientOverviewLink) setTrainerRevokeLink(clientOverviewLink); }} />}
