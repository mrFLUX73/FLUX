...
function ClientOverviewDrawer({ open, onOpenChange, userId, link, onOpenChat }: { open: boolean; onOpenChange: (open: boolean) => void; userId: string; link: TrainerLink | null; onOpenChat: () => void }) {
  ...
  useEffect(() => { if (!open || !link) return; setLoading(true); setError(''); void loadTrainerClientOverview(userId, link.id).then(setOverview).catch(() => setError('Не удалось загрузить данные клиента. Обновите связь с клиентом и попробуйте ещё раз.')).finally(() => setLoading(false)); }, [link, open, userId]);
